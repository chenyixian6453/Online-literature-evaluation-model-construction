...
# 1. 替换导入：删除 SimpleQidianCrawler，导入 MobileQidianCrawler
from qidian_crawler_content_analysis_final import MobileQidianCrawler  # 注意文件名对应你的实际文件（去掉括号和final）
...
class CrawlerScheduler:
    def __init__(self):
        # 2. 替换爬虫实例：SimpleQidianCrawler → MobileQidianCrawler
        self.chapter_crawler = MobileQidianCrawler(headless=False)  # 移动端爬虫需要指定headless参数
    ...
    def crawl_single_novel_comprehensive(self, work_id: int, work_name: str, work_url: str):
        ...
        try:
            # 3. 替换爬取方法：crawl_novel → crawl_mobile_novel
            #    参数调整：MobileQidianCrawler的crawl_mobile_novel接收book_id和max_chapters
            chapter_success = self.chapter_crawler.crawl_mobile_novel(
                book_id=work_id,  # 移动端爬虫用book_id（和work_id一致）
...
def main():
    ...
    if choice == "1":
        ...
        # 测试完成后关闭浏览器
        scheduler.chapter_crawler.close()
